...
def step4_results(routes, n_no_vehicle, n_safety_fail_dispatched, m_th):
    ...
    add("")
    add("DATA FINDING - energy is vehicle-independent in this dataset:")
    add("  The GBM gives 0.000 importance to mass_kg, battery_capacity_wh,")
    add("  max_payload_kg and base_efficiency_mult; adding vehicle identity")
    add("  dummies does not improve held-out MAE (193.5 vs 193.3 Wh); GBM")
    add("  residuals differ by only 30 Wh across vehicles (within-vehicle")
    add("  std 130-183 Wh). Trip energy is a function of route + payload")
    add("  only, so there is no meaningful energy comparison between vehicles")
    add("  for the same route.")
